...
# -------------------------------------------------
#  SIMPLE WEB SERVER (keeps Render happy)
# -------------------------------------------------
app = Flask(__name__)
...
# -------------------------------------------------
#  BOT SETUP  (must come before any @bot decorators)
# -------------------------------------------------
...
# These must be after bot is defined
app_install_users: set[int] = set()
...
async def apply_discord_roles_and_nick(
    ...
    *,
    ...
    desired_role_objects = [guild.get_role(rid) for rid in desired_ids if guild.get_role(rid) is not None]
...
async def before_weekly_tiktok():
    await bot.wait_until_ready()
    print("TikTok reminder task started (every 3 days at 21:30 UK)")
...
async def open_roles(interaction: discord.Interaction):
    ...
    try:
        ...
        if singleton_open:
            add_chunked_field(embed, f"✅ Open Positions ({len(singleton_open)})", "✅ Open Positions (cont.)", singleton_open)
        else:
        ...
        if singleton_filled:
            add_chunked_field(embed, f"❌ Filled Positions ({len(singleton_filled)})", "❌ Filled Positions (cont.)", singleton_filled)

        embed.set_footer(text=f"{len(singleton_open)} open · {len(singleton_filled)} filled · {len(infinite_roles)} unlimited")
        ...
        if status == 200 and "error" not in response_text.lower():
            ...
            if field == "role":
                ...
                if roblox_username:
                    ...
                    embed.add_field(name="Roblox Group", value=f"✅ Ranked to **{result}**" if success else f"⚠️ {result}", inline=False)
                else:
                    embed.add_field(name="Roblox Group", value="⚠️ No Roblox username on file", inline=False)

            guild = interaction.guild
            if guild:
                target_discord_id = value if field == "discord_id" else await get_discord_id_for_staff(staff_name)
                if target_discord_id and target_discord_id != "N/A":
                    member = await get_discord_member_by_id(guild, target_discord_id)
                    if member:
                        if field == "role":
                            effective_role, effective_teaching_name = value, staff_name
                        ...
                        if effective_role and effective_role != "N/A":
                            await apply_discord_roles_and_nick(guild, member, effective_role, effective_teaching_name or staff_name, embed=embed)
                        else:
...
class LoaEndDateSelect(discord.ui.Select):
    def __init__(self, staff_name: str, reason: str, start_date_str: str, invoker_id: int):
        ...
        for i in range(1, 26):
            ...
            options.append(discord.SelectOption(label=d.strftime("%d %B %Y"), value=d.strftime("%Y-%m-%d")))

        super().__init__(placeholder="Select LOA end date...", min_values=1, max_values=1, options=options, custom_id="loa_end_date_select")

    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.invoker_id:
            return await interaction.response.send_message("❌ This isn't your LOA form.", ephemeral=True)
        ...
        params = {"action": "loa", "staffName": self.staff_name, "loaValue": loa_value, "loaReason": self.reason.strip()}
        ...
        try:
            status, response_text = await safe_apps_script_get(APPS_SCRIPT_URL, params)
            embed.add_field(name="Sheet", value="✅ Recorded" if (status == 200 and "error" not in response_text.lower()) else f"⚠️ HTTP {status}: {response_text[:200]}", inline=True)
        except Exception as e:
        ...
        if guild and sheet_role and sheet_role != "N/A":
            await apply_loa_discord(guild, self.staff_name, sheet_role, discord_id or "N/A", embed=embed)
        else:
...
class LoaStartDateSelect(discord.ui.Select):
    def __init__(self, staff_name: str, reason: str, invoker_id: int):
        ...
        for i in range(4):
            ...
            label = d.strftime("%d %B %Y") + (" (Today)" if i == 0 else "")
            options.append(discord.SelectOption(label=label, value=d.strftime("%Y-%m-%d")))

        super().__init__(placeholder="Select LOA start date...", min_values=1, max_values=1, options=options, custom_id="loa_start_date_select")
...
@app_commands.describe(staff_name="Select the staff member", reason="Reason for the leave of absence")
@app_commands.autocomplete(staff_name=edit_staff_autocomplete)
@cooldown()
async def set_loa(interaction: discord.Interaction, staff_name: str, reason: str):
    if not reason or not reason.strip():
        return await interaction.response.send_message("❌ A reason is required.", ephemeral=True)

    view = LoaStartDateView(staff_name=staff_name, reason=reason, invoker_id=interaction.user.id)
...
async def end_loa(interaction: discord.Interaction, staff_name: str):
    ...
    target_row = None
    for row in all_data[CURRENT_STAFF_DATA_START - 1:]:
        if len(row) > CURRENT_STAFF_NAME_COL and row[CURRENT_STAFF_NAME_COL].strip().lower() == staff_name.strip().lower():
            target_row = row
            break
    ...
    if not loa_value or loa_value == "N/A":
        await interaction.followup.send(f"❌ **{staff_name}** does not currently have an active LOA on record.")
        return

    embed = discord.Embed(title="✅ Leave of Absence Ended", color=discord.Color.green(), timestamp=datetime.now())
    embed.add_field(name="Staff Member", value=staff_name,              inline=True)
    ...
    try:
        ...
        status, resp_text = await safe_apps_script_get(APPS_SCRIPT_URL, params)
        embed.add_field(name="Sheet", value="✅ LOA cleared" if (status == 200 and "error" not in resp_text.lower()) else f"⚠️ HTTP {status}: {resp_text[:200]}", inline=False)
    except Exception as e:
        embed.add_field(name="Sheet", value=f"⚠️ Error: {e}", inline=False)

    guild = interaction.guild
    if guild:
        await remove_loa_discord(guild, staff_name, sheet_role, discord_id, embed=embed)
    else:
...
async def _execute_hire(
    ...
    try:
        ...
        if status == 200 and "error" not in response_text.lower():
            ...
            guild = interaction.guild
            if guild:
                await apply_discord_roles_and_nick(guild, discord_account, role, teaching_name, embed=embed)
...
class HireOverrideView(discord.ui.View):
    def __init__(self, hiring_staff, teaching_name, roblox_username, discord_account, area, role, block_reason):
    ...
    async def _disable_all(self, interaction, new_title, new_color):
        for child in self.children:
            child.disabled = True
    ...
    async def override(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        await self._disable_all(interaction, "⚠️ Override Approved — Hiring...", discord.Color.orange())

        override_note = discord.Embed(title="⚠️ Eligibility Override Used", color=discord.Color.orange(), timestamp=datetime.now())
        ...
        await _execute_hire(interaction, self.teaching_name, self.roblox_username, self.discord_account, self.area, self.role)
    ...
    async def no_override(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        await self._disable_all(interaction, "❌ Hire Cancelled", discord.Color.red())
        await interaction.followup.send(f"Hire cancelled. {self.discord_account.mention} was **not** added to the staff team.", ephemeral=True)
...
async def hire(interaction: discord.Interaction, teaching_name: str, roblox_username: str, discord_account: discord.Member, area: str, role: str):
    ...
    if not can_hire:
        reason_label = "🚫 User is on the staff blacklist" if block_reason == "blacklisted" else "⏳ User was removed from staff within the last 2 weeks"
        embed_color  = discord.Color.red() if block_reason == "blacklisted" else discord.Color.orange()

        await interaction.followup.send("Whoops! Looks like you can't hire them right now. Try again later!", ephemeral=True)

        block_embed = discord.Embed(title="⛔ Hire Blocked — Eligibility Check Failed", color=embed_color, timestamp=datetime.now())
        ...
        view = HireOverrideView(interaction.user, teaching_name, roblox_username, discord_account, area, role, reason_label)
        ...
        if status == 200 and "error" not in response_text.lower():
            ...
            if target_discord_id and target_discord_id != "N/A":
                try:
                    ...
                    embed.add_field(name="RSL Discord ID", value=f"✅ Written to Column E: `{target_discord_id}`" if wrote else "⚠️ Could not find matching row in Remove Staff Log", inline=False)
                except Exception as e:
            ...
            if roblox_username:
                ...
                embed.add_field(name="Roblox Group", value="✅ Demoted to rank 1" if success else f"⚠️ {err}", inline=False)
            else:
                embed.add_field(name="Roblox Group", value="⚠️ No Roblox username on file", inline=False)
            ...
            if guild and target_discord_id and target_discord_id != "N/A":
                member = await get_discord_member_by_id(guild, target_discord_id)
                if member:
                    await clear_discord_roles_and_reset_nick(guild, member, roblox_username or staff_name, embed=embed)
                else:
                    embed.add_field(name="Discord", value="⚠️ Member not found in this server", inline=False)
            else:
                embed.add_field(name="Discord", value="⚠️ No Discord ID on file — skipped role/nick update", inline=False)
...
async def view_staff(interaction: discord.Interaction):
    ...
    try:
        all_data   = await safe_sheets_call(lambda: spreadsheet.worksheet(CURRENT_STAFF_SHEET).get_all_values())
        roles_data = await safe_sheets_call(lambda: spreadsheet.worksheet(ROLES_LIST_SHEET).get_all_values())
        ...
        first_embed = discord.Embed(title="👥 Current Staff", description=chunks[0], color=discord.Color.blue(), timestamp=datetime.now())
        first_embed.set_footer(text=f"{total} staff member(s) total")
        await interaction.followup.send(embed=first_embed)

        for i, chunk in enumerate(chunks[1:], start=2):
            cont_embed = discord.Embed(description=chunk, color=discord.Color.blue())
...
@app_commands.describe(discord_account="The Discord user to blacklist", roblox_username="Their Roblox username", reason="Reason for blacklisting")
@cooldown()
async def staff_blacklist(interaction: discord.Interaction, discord_account: discord.Member, roblox_username: str, reason: str):
    await interaction.response.defer()
    ...
    try:
        def _add_to_blacklist():
            ...
            all_data = ws.get_all_values()
            filled   = sum(1 for row in all_data[BL_DATA_START - 1:] if any(cell.strip() for cell in row[:5]))
        ...
        embed = discord.Embed(title="🚫 Staff Blacklisted", color=discord.Color.red(), timestamp=datetime.now())
...
# ── Autocomplete for /unblackliststaff ──
async def blacklisted_staff_autocomplete(interaction: discord.Interaction, current: str):
    try:
        def _get_blacklisted():
            ws = spreadsheet.worksheet(BLACKLISTED_STAFF_SHEET)
            all_data = ws.get_all_values()
            entries = []
            for row in all_data[BL_DATA_START - 1:]:
                discord_id = safe_get(row, BL_DISCORD_ID_COL)
                roblox     = safe_get(row, BL_ROBLOX_COL)
                if discord_id:
                    entries.append((discord_id, roblox))
            return entries

        entries = await safe_sheets_call(_get_blacklisted)
        choices = []

        for discord_id, roblox in entries:
            try:
                member = interaction.guild.get_member(int(discord_id))
                username = f" [{member.name}]" if member else ""
            except (ValueError, AttributeError):
                username = ""

            label = f"{roblox}{username}"

            if current.lower() in label.lower() or current in discord_id:
                choices.append(app_commands.Choice(name=label, value=discord_id))
...
async def unblacklist_staff(interaction: discord.Interaction, discord_id: str):
    ...
    try:
        ...
        if found:
            ...
            embed = discord.Embed(title="✅ Staff Unblacklisted", color=discord.Color.green(), timestamp=datetime.now())
            embed.add_field(name="Discord ID",              value=discord_id,     inline=True)
...
async def sync_all_staff(interaction: discord.Interaction, dry_run: bool = False):
    ...
    rows = [row for row in all_data[CURRENT_STAFF_DATA_START - 1:] if len(row) > CURRENT_STAFF_NAME_COL and row[CURRENT_STAFF_NAME_COL].strip()]
    ...
    total        = len(rows)
    success_disc = failed_disc = success_rbx = failed_rbx = skipped = 0
    details      = []

    await interaction.followup.send(f"{'🔍 [DRY RUN] ' if dry_run else ''}⏳ Syncing **{total}** staff members... this may take a while.", ephemeral=True)
        ...
        if roblox_username and roblox_username != "N/A":
            if sheet_role in ROLE_NAME_MAP:
                if not dry_run:
                    ...
                    line_parts.append(f"🔍 Roblox → {ROLE_NAME_MAP.get(sheet_role, 'unknown')}")
                    success_rbx += 1
            else:
    ...
    embed.set_footer(text="Dry run — no changes were made." if dry_run else f"Triggered by {interaction.user.display_name}")
...
async def sync_staff(interaction: discord.Interaction, staff_name: str):
    ...
    if target_row is None:
        await interaction.followup.send(f"❌ Staff member **{staff_name}** not found in {CURRENT_STAFF_SHEET}.", ephemeral=True)
        return
    ...
    embed = discord.Embed(title=f"🔄 Syncing: {teaching_name}", color=discord.Color.blue(), timestamp=datetime.now())
    ...
    if discord_id_str and discord_id_str != "N/A":
        ...
        if member:
            if sheet_role and sheet_role != "N/A":
                await apply_discord_roles_and_nick(guild, member, sheet_role, teaching_name, embed=embed)
            else:
    ...
    if roblox_username and roblox_username != "N/A":
        if sheet_role in ROLE_NAME_MAP:
            success, result = await roblox_set_rank_by_sheet_role(roblox_username, sheet_role)
            embed.add_field(name="Roblox", value=f"✅ Ranked to **{result}**" if success else f"⚠️ {result}", inline=False)
        else:
...
# ── /viewremovedstaff ──
async def removed_staff_autocomplete(interaction: discord.Interaction, current: str) -> list[app_commands.Choice[str]]:
    try:
        all_data = await safe_sheets_call(lambda: spreadsheet.worksheet(REMOVE_STAFF_LOG_SHEET).get_all_values())
        names    = []
    ...
    try:
        ...
        for row in all_data[RSL_DATA_START - 1:]:
            ...
            if name.strip().lower() == staff_name.strip().lower():
                records.append((name, safe_get(row, RSL_DATE_COL), safe_get(row, RSL_DISCORD_ID_COL), safe_get(row, RSL_REASON_COL)))

        if not records:
            await interaction.followup.send(f"❌ No removal record found for **{staff_name}**.", ephemeral=False)
            return

        embed = discord.Embed(title=f"🗂️ Removal Record — {staff_name}", color=discord.Color.red(), timestamp=datetime.now())
        ...
        for i, (name, date, discord_id, reason) in enumerate(records, start=1):
            ...
            embed.add_field(name=f"{label} — Discord ID",      value=f"<@{discord_id}>" if discord_id != "N/A" else "N/A", inline=True)
...
async def request_display_name(interaction: discord.Interaction, new_name: str):
    ...
    request_embed = discord.Embed(title="🎓 Student Name Change Request", description="A student has requested a name change.", color=discord.Color.blue(), timestamp=datetime.now())
    ...
    view = StudentNameRequestView(requester_id=interaction.user.id, requested_name=new_name)
...
class CustomRoleView(discord.ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        color_value = discord.Color(int(clean_hex, 16)) if is_valid_hex(clean_hex) else discord.Color.default()

        try:
            new_role = await guild.create_role(name=self.role_name, color=color_value, reason=f"Custom role approved for {member.display_name}")
            await member.add_roles(new_role)
            dm_embed = discord.Embed(title="Custom Role Approved!", description=f"Your request for the custom role **{self.role_name}** has been approved.\n\nThe role has been created and added to your profile. Enjoy your new look!", color=color_value)
            await member.send(embed=dm_embed)
    ...
    async def deny(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            member = await guild.fetch_member(self.requester_id)
            if member:
                dm_embed = discord.Embed(title="Role Request Denied", description=f"Unfortunately, your request for the custom role **{self.role_name}** was not approved at this time.\n\nIf you have questions, please contact the Senior Leadership Team.", color=discord.Color.red())
                await member.send(embed=dm_embed)
        except Exception:
            pass
...
async def customrolerequest(interaction: discord.Interaction, name: str, color: str = None):
    ...
    request_embed = discord.Embed(title="🎨 New Custom Role Request", color=preview_color, timestamp=interaction.created_at)
...
async def _send_blocked_notification(client, user, discord_id, block_reason, roblox_username="N/A", teaching_name="N/A", stage="/apply command"):
    ...
    blocked_embed = discord.Embed(title="⛔ Blocked Application Attempt", description=f"A user was automatically blocked at the **{stage}** stage.", color=embed_color, timestamp=datetime.now())
...
class ApplicationModal(discord.ui.Modal, title="Winstree Academy - Teaching Staff Application"):
    roblox_username = discord.ui.TextInput(label="Roblox Username", placeholder="Enter your exact Roblox username", required=True, max_length=50)
    teaching_name   = discord.ui.TextInput(label="Full teaching name - initials will be denied.", placeholder="e.g. Miss Zoe Parker", required=True, max_length=60)
    reason          = discord.ui.TextInput(label="Why do you want to be staff?", placeholder="Write a couple of sentences about why you'd like to join the team...", style=discord.TextStyle.paragraph, required=True, min_length=30, max_length=500)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not can_apply:
            try:
                ...
                pass
            await interaction.followup.send("Oops! You can't apply for that position right now. Try again later!", ephemeral=True)
            await _send_blocked_notification(client=interaction.client, user=interaction.user, discord_id=discord_id, block_reason=block_reason, roblox_username=self.roblox_username.value, teaching_name=self.teaching_name.value, stage="modal submission")
            return
        ...
        embed = discord.Embed(title="📋 New Staff Application", color=discord.Color.orange(), timestamp=datetime.now())
        ...
        view = ApplicationReviewView(applicant_id=interaction.user.id, roblox_username=self.roblox_username.value, teaching_name=self.teaching_name.value, age_range=self.age_range, reason=self.reason.value)
        await log_channel.send(content="@here", embed=embed, view=view)
        await interaction.followup.send("✅ Your application has been submitted! You will receive a DM once it has been reviewed by the Senior Leadership Team.", ephemeral=True)

    async def on_error(self, interaction: discord.Interaction, error: Exception):
        print(f"[Apply] Modal error: {error}")
        try:
            await interaction.response.send_message("❌ Something went wrong submitting your application. Please try again.", ephemeral=True)
        except Exception:
            pass
...
class AgeRangeSelect(discord.ui.Select):
    def __init__(self):
        ...
        super().__init__(placeholder="Select your age range to begin...", min_values=1, max_values=1, options=options, custom_id="age_range_select")

    async def callback(self, interaction: discord.Interaction):
        if self.values[0] == "U9":
            await interaction.response.send_message("❌ You must be aged 9 or over to apply for a staff position at Winstree Academy.", ephemeral=True)
            return
        await interaction.response.send_modal(ApplicationModal(age_range=self.values[0]))
...
class AgeRangeView(discord.ui.View):
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception, item):
        ...
        try:
            await interaction.response.send_message("❌ Something went wrong. Please try running /apply again.", ephemeral=True)
        except Exception:
            pass


class ApplicationReviewView(discord.ui.View):
    def __init__(self, applicant_id, roblox_username, teaching_name, age_range, reason):
        super().__init__(timeout=None)
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        area = "Academy" if self.age_range in ("9-12", "13-15", "16-17") else "Sixth Form"
        role = "School Staff"

        sheet_status = ""
        params = {"action": "hire", "teachingName": self.teaching_name, "staffUsername": self.roblox_username, "area": area, "position": role, "discordId": str(self.applicant_id)}
        try:
            status, response_text = await safe_apps_script_get(APPS_SCRIPT_URL, params)
            sheet_status = "✅ Added to sheet" if (status == 200 and "error" not in response_text.lower()) else f"⚠️ Sheet error (HTTP {status}): {response_text[:200]}"
        except Exception as e:
        ...
        try:
            ...
            roblox_status = f"✅ Ranked to **{result}**" if success else f"⚠️ {result}"
        except Exception as e:
        ...
        if guild and member:
            try:
                ...
                discord_status = f"✅ Roles: {', '.join(r.name for r in desired_role_objects) or 'None'}\n✅ Nickname: `{new_nick}`"
            except discord.Forbidden:
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception, item):
        ...
        try:
            await interaction.followup.send("❌ Something went wrong processing this application. Please try again.", ephemeral=True)
        except Exception:
            pass
...
async def apply(interaction: discord.Interaction):
    ...
    if not can_apply:
        ...
        await _send_blocked_notification(client=interaction.client, user=interaction.user, discord_id=discord_id, block_reason=block_reason, stage="/apply command")
        return await interaction.response.send_message("Oops! You can't apply for that position right now. Try again later!", ephemeral=True)
...
async def text_readsheet(ctx, sheet_name: str = "Sheet1"):
    try:
        all_data = await safe_sheets_call(lambda: spreadsheet.worksheet(sheet_name).get_all_values())
        if not all_data:
